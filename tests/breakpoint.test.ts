...
describe('breakpoint', () => {
  ...
  it('check min with integer', done => {
    expect(`${breakpoint([MOBILE_INT[0]])} { background-color: red; }`).toEqual(
      `@media only screen and (min-width: ${MOBILE_STR[0]}) { background-color: red; }`,
    );
    done();
  });
  ...
  it('check max with integer', done => {
    expect(`${breakpoint([undefined, MOBILE_INT[1]])} { background-color: red; }`).toEqual(
  ...
  it('check min & max with integer', done => {
    expect(`${breakpoint([MOBILE_INT[0], MOBILE_INT[1]])} { background-color: red; }`).toEqual(
  ...
  it('check min with float', done => {
    expect(`${breakpoint([MOBILE_FLT[0]])} { background-color: red; }`).toEqual(
  ...
  it('check max with float', done => {
    expect(`${breakpoint([undefined, MOBILE_FLT[1]])} { background-color: red; }`).toEqual(
  ...
  it('check min & max with float', done => {
    expect(`${breakpoint([MOBILE_FLT[0], MOBILE_FLT[1]])} { background-color: red; }`).toEqual(
  ...
  it('check min with string', done => {
    expect(`${breakpoint([MOBILE_STR[0]])} { background-color: red; }`).toEqual(
      `@media only screen and (min-width: ${MOBILE_STR[0]}) { background-color: red; }`,
    );
    done();
  });
  ...
  it('check max with string', done => {
    expect(`${breakpoint([undefined, MOBILE_STR[1]])} { background-color: red; }`).toEqual(
      `@media only screen and (max-width: ${MOBILE_STR[1]}) { background-color: red; }`,
    );
    done();
  });
  ...
  it('check min & max with string', done => {
    expect(`${breakpoint(MOBILE_STR)} { background-color: red; }`).toEqual(
      `@media only screen and (min-width: ${MOBILE_STR[0]}) and (max-width: ${MOBILE_STR[1]}) { background-color: red; }`,
    );
    done();
  });
  ...
  it('Result template', done => {
    ...
    const padding = '10px';
    expect(
      `${breakpoint([1, 2])} { background-color: ${color}; color: ${color}; height: ${1}px; width: ${1 +
